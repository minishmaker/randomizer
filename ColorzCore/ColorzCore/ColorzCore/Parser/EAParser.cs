...
namespace ColorzCore.Parser
{
	internal class EAParser
	{
		...
		private Maybe<StatementNode> ParseStatement(MergeableGenerator<Token> tokens, ImmutableStack<Closure> scopes)
		{
			...
			var upperCodeIdentifier = head.Content.ToUpperInvariant();
			...
			bool expandDefs = true)
		{
			...
			var head = tokens.Current;
			var grammarSymbols = new Stack<Either<IAtomNode, Token>>();
			var ended = false;
			while (!ended)
			{
		...
		public Maybe<ILineNode> ParseLine(MergeableGenerator<Token> tokens, ImmutableStack<Closure> scopes)
		{
			if (IsIncluding)
			{
				...
				switch (head.Type)
				{
						...
						if (tokens.Current.Type == TokenType.COLON)
						{
							...
							if (scopes.Head.HasLocalLabel(head.Content))
								Warning(head.Location,
									...
									head.Content); //replacing: " + head.Content);
					...
					default:
						tokens.MoveNext();
						Error(head.Location, string.Format("Unexpected token: {0}: {1}", head.Type, head.Content));
						IgnoreRestOfLine(tokens);
						break;
				}
		...
		private Maybe<ILineNode> ParsePreprocessor(MergeableGenerator<Token> tokens, ImmutableStack<Closure> scopes)
		{
			...
			var retVal = directiveHandler.HandleDirective(this, head, paramList, tokens);
			if (!retVal.IsNothing)
			{
