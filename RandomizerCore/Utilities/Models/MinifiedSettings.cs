...
{
	/// <summary>
	/// A minified version of settings strings that can be shared. These require that the logic file crc32 matches, otherwise it will fail to load.
	...
	{
		private static byte[] _header = new byte[] { 0x36, 0x58, 0x02 };

		public static void GenerateSettingsFromBase64String(string settingString, List<LogicOptionBase> sortedLogicOptions, uint logicOptionsCrc32)
		{
			if (!settingString.Substring(0, 4).Equals("NlgC", StringComparison.Ordinal)) throw new InvalidSettingStringException("Tried to load minified settings string but identifier NlgC was not found!");
			...
			if (crc32 != logicOptionsCrc32) throw new InvalidSettingStringException("Tried to load minified settings string with the wrong logic file!");

			currentIndex = 7;
			var currentByte = bytes[currentIndex];
			var optionGroups = sortedLogicOptions.GroupBy(x => x.GetType()).ToList();

			var flags = optionGroups.Any(group => group.Key == typeof(LogicFlag)) ? optionGroups.First(group => group.Key == typeof(LogicFlag)).Cast<LogicFlag>().ToList() : new List<LogicFlag>();
			...
			var dropdowns = optionGroups.Any(group => group.Key == typeof(LogicDropdown)) ? optionGroups.First(group => group.Key == typeof(LogicDropdown)).Cast<LogicDropdown>().ToList() : new List<LogicDropdown>();
			...
			var numberBoxes = optionGroups.Any(group => group.Key == typeof(LogicNumberBox)) ? optionGroups.First(group => group.Key == typeof(LogicNumberBox)).Cast<LogicNumberBox>().ToList() : new List<LogicNumberBox>();

			for (int numberBoxesProcessed = 0; numberBoxesProcessed < numberBoxes.Count; ++numberBoxesProcessed)
			{
				currentByte = bytes[++currentIndex];
			...
			var colorPickers = optionGroups.Any(group => group.Key == typeof(LogicColorPicker)) ? optionGroups.First(group => group.Key == typeof(LogicColorPicker)).Cast<LogicColorPicker>().ToList() : new List<LogicColorPicker>();

			for (int colorPickersProcessed = 0; colorPickersProcessed < colorPickers.Count; ++colorPickersProcessed)
			{
				currentByte = bytes[++currentIndex];
				...
				colorPicker.DefinedColor = new GBAColor((currentByte & 0xF8) >> 3, ((currentByte & 0x7) << 2) | ((colorSecondByte & 0xC0) >> 6), (colorSecondByte & 0x3E) >> 1).ToColor();
		...
		public static string GenerateSettingsString(List<LogicOptionBase> sortedLogicOptions, uint logicOptionsCrc32)
		{
			...
			for (int colorPickersProcessed = 0; colorPickersProcessed < colorPickers.Count; ++colorPickersProcessed)
			{
				...
				colorSecondByte |= colorPicker.UseRandomColor ? (byte)1 : (byte)0;
