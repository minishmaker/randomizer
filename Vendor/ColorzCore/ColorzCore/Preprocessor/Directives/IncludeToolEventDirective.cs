...
{
    internal class IncludeToolEventDirective : IDirective
    {
        ...
        public IMaybe<ILineNode> Execute(EaParser parse, Token self, IList<IParamNode> parameters,
            MergeableGenerator<Token> tokens)
        {
            ...
            else
            {
                ...
                var t = new Tokenizer();
                tokens.PrependEnumerator(t.Tokenize(outputBytes,
                        Path.GetFileName(self.FileName) + ", Line " + self.LineNumber + "; " + parameters[0].ToString())
                    .GetEnumerator());
            }
